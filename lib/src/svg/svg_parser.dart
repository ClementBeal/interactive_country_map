...
class SvgPath {
  ...
  Path toPath({Size? originalMapSize, Size? maxSize}) {
    ...
    for (final point in points) {
      switch (point) {
        ...
        // because the path is relative, we only add the offset to the first point
          ...
          for (final relativePoint in point.relativePoints) {
            path.relativeLineTo(relativePoint.x, relativePoint.y);
          }
          break;
        case final LinePoint point:
          if (point.isRelative) {
            path.relativeLineTo(point.x, point.y);
          } else {
            path.lineTo(point.x, point.y);
          }
        case final CurvePoint point:
          if (point.isRelative) {
    ...
    if (maxSize != null && originalMapSize != null) {
      final double scaleX = maxSize.width / originalMapSize.width;
      final double scaleY = maxSize.height / originalMapSize.height;
...
class SvgParser {
  ...
  Future<CountryMap> parse(String data) async {
    ...
    final countryPaths =
    xml.findAllElements('path').map(_getCountryPath).toList();
